...
public fun files(glob: String, `type`: ArtifactType, base: Path? = null): Artifact =
        FileGlobArtifact(base ?: Paths.get(glob).getParent(),`type`).let { it.include(glob); it }
class FileGlobArtifact(base: Path, type: ArtifactType) : FolderArtifact(base, type) {
    ...
    override val name: String = "$`type` glob +$included -$excluded"
}
