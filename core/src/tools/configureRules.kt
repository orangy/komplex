...
fun<R: Rule> R.configureSingleFolderTarget(module: komplex.dsl.ProjectModule, type: ArtifactType, nameGen: () -> String = { module.name }): BuildDiagnostic =
...
fun<R: Rule> R.configureSingleTempFolderTarget(module: komplex.dsl.ProjectModule, type: ArtifactType, nameGen: () -> String = { module.name }): BuildDiagnostic =
...
fun<R: Rule> R.configureSingleFileTarget(module: komplex.dsl.ProjectModule, type: ArtifactType, nameGen: () -> String = { module.name }): BuildDiagnostic =
    if (configureSingleInto(module.env.defaultTargetDir, { makeDefaultFileArtifact(module, type, it, nameGen) }))
        BuildDiagnostic.Success
    else BuildDiagnostic.Fail("$name (${module.fullName}) Cannot auto configure target folder: defaultTargetDir is not defined")


fun<R: Rule> R.configureSingleTempFileTarget(module: komplex.dsl.ProjectModule, type: ArtifactType, nameGen: () -> String = { module.name }): BuildDiagnostic =
